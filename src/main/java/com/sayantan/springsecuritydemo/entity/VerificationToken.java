...
public class VerificationToken {
    ...
    private User user;       //for each particular user, a unique token will be stored in the database. Hence we need to do one to one mapping the database with the User table and the verificationToken Table

    public VerificationToken(User user,String token){               //Here we are not using the lombok annotations for COnstructore and we want to define the constructor by yourself
        super();
        this.user=user;
        this.token=token;
